...
const PointsByPaymentMethod = ({
  transactions,
}: {
  transactions: Transaction[];
}) => {
  const pointsByMethod = transactions.reduce<
    Record<string, { points: number; currency: string }>
  >((acc, transaction) => {
    if (!transaction.paymentMethod || !transaction.rewardPoints) return acc;

    const methodName = transaction.paymentMethod.name;
    const pointsCurrency = rewardService.getPointsCurrency(
      ...
      {Object.entries(pointsByMethod).map(([method, { points, currency }]) => (
        ...
        >
          ...
          <span className="font-medium">
            {points.toLocaleString()} {currency}
          </span>
        </div>
      ))}
    </div>
...
const RewardPoints = () => {
  const { transactions, filteredTransactions } = useTransactionList();
