...
export function CardBarRow({
  ...
  const barWidth = hasConversion && maxMiles > 0 
    ? (convertedMiles / maxMiles) * 100 
    : 0;
  
  // Format miles value for display
  const formattedMiles = convertedMiles !== null 
    ? Math.round(convertedMiles).toLocaleString() 
    : '—';
