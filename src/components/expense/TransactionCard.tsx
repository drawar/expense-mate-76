    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <h3 className="font-medium text-lg line-clamp-1">{merchant.name}</h3>
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 flex flex-wrap gap-2">
        <div className="flex items-center text-sm rounded-full px-3 py-1 bg-gray-100 dark:bg-gray-800">
          {paymentMethod.type === 'credit_card' ? (
            <CreditCardIcon className="h-3.5 w-3.5 mr-1.5" style={{ color: paymentMethod.color }} />
          ) : (
            <BanknoteIcon className="h-3.5 w-3.5 mr-1.5" style={{ color: paymentMethod.color }} />
          )}
        ...
        {merchant.mcc && (
          <div className="flex items-center text-sm rounded-full px-3 py-1 bg-gray-100 dark:bg-gray-800">
            <TagIcon className="h-3.5 w-3.5 mr-1.5 text-purple-500" />
            <span className="truncate max-w-[120px]">{merchant.mcc.description}</span>
          </div>
        )}

        {rewardPoints > 0 && (
          <div className="flex items-center text-sm rounded-full px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
            <span>+{rewardPoints} points</span>
          </div>
        )}

        {merchant.address && (
          <div className="flex items-center text-sm rounded-full px-3 py-1 bg-gray-100 dark:bg-gray-800 w-full mt-1">
            <MapPinIcon className="h-3.5 w-3.5 mr-1.5 text-red-500" />
