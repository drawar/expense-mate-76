...
const PaymentDetailsForm = ({ 
  ...
  useEffect(() => {
    ...
    } else if (selectedPaymentMethod?.issuer === 'Citibank' && selectedPaymentMethod?.name === 'Rewards Visa Signature') {
      const allTransactions = getTransactions();
      const currentDate = new Date();
      let totalMonthBonusPoints = 0;
  ...
  const getPointsMessage = () => {
    ...
    if (selectedPaymentMethod?.issuer === 'UOB' && 
      ...
      if (currency !== 'SGD') {
        const sgdEquivalent = shouldOverridePayment ? paymentAmount : amount * 1.35;
    ...
    if (selectedPaymentMethod?.issuer === 'Citibank' && 
      ...
      const exclusionMCCs = ['3000', '3001', '3002', '3003', '3004', '3005', '3006', '3007', '3008', '3009', 
      ...
      const isEligibleMCC = mcc && inclusionMCCs.includes(mcc);
      const isExcludedMCC = mcc && exclusionMCCs.includes(mcc);
      const isEligibleTransaction = (isOnline && !isExcludedMCC) || isEligibleMCC;
