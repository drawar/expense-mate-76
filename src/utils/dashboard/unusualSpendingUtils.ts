...
/**
 * Hook-like function for unusual spending detection
 * Returns anomalies and alert count based on transaction data
 */
export function analyzeUnusualSpending(
  ...
  // Get recent transactions for analysis (last 30 days)
  const recentTransactions = transactions.filter(
    tx => new Date(tx.date) >= new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
  );
  
  // Get historical transactions for baseline (excluding most recent week)
