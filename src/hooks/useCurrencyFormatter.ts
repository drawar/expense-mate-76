...
export function useCurrencyFormatter(defaultCurrency: Currency = 'SGD') {
  ...
  const formatCurrency = useCallback((amount: number, currency: Currency = defaultCurrency): string => {
  ...
  const formatCurrencyBatch = useCallback((amounts: number[], currency: Currency = defaultCurrency): string[] => {
    console.log(`Batch formatting ${amounts.length} currency values`);
    return amounts.map(amount => formatCurrencyUtil(amount, currency));
  }, [defaultCurrency]);
  ...
  const cachedFormatters = useMemo(() => {
    ...
    return {
      format: (amount: number, currency: Currency = defaultCurrency): string => {
        const key = `${amount}-${currency}`;
        if (!cache.has(key)) {
          cache.set(key, formatCurrencyUtil(amount, currency));
        }
...
export function useChartCurrencyFormatter(currency: Currency = 'SGD') {
  ...
  const tooltipFormatter = useCallback((value: number, name: string) => {
    return [formatCurrencyUtil(value, currency), name];
  }, [currency]);
