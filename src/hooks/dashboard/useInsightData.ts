...
import type { RewardRule } from "@/core/rewards/types";
import {
  ...
  return useMemo(() => {
    ...
    activePaymentMethods.forEach((method) => {
      ...
      if (method.rewardRules) {
        (method.rewardRules as RewardRule[]).forEach((rule) => {
            ...
            if (
              ...
              condition.values.forEach((val) => {
                if (typeof val === "string") {
                  ...
                  categoryRules.set(
                    valLower,
                    Math.max(categoryRules.get(valLower) || 0, multiplier)
                  );
                }
              });
            }
    ...
    categoryDataMap.forEach((catData, category) => {
      ...
      methodRewardMap.forEach((methodInfo, methodName) => {
        ...
        if (methodInfo.categoryRules.has(categoryLower)) {
          potentialReward = Math.max(
            potentialReward,
            methodInfo.categoryRules.get(categoryLower)!
          );
        } else {
          // Check partial matches if no exact match found
          methodInfo.categoryRules.forEach((rate, ruleCat) => {
            if (
              categoryLower.includes(ruleCat) ||
              ruleCat.includes(categoryLower)
            ) {
      ...
      if (bestMethod !== currentMethod) {
        ...
        const currentReward =
          currentMethodInfo?.defaultRate || DEFAULT_REWARD_RATE;
  ...
  return useMemo(() => {
    ...
    transactions.forEach((tx) => {
      ...
      if (!catData) {
        ...
        const isDiscretionary =
          DISCRETIONARY_CATEGORIES.has(categoryLower) ||
          Array.from(DISCRETIONARY_CATEGORIES).some(
            (c) => categoryLower.includes(c) || c.includes(categoryLower)
          );
      ...
      const savingsRate = catData.discretionary
        ? DISCRETIONARY_SAVINGS_RATE
        : ESSENTIAL_SAVINGS_RATE;
    ...
    const savingsProgress = Math.min(
      ...
      totalSavingsPotential > 0 && savingsTarget > 0
        ? (totalSavingsPotential / savingsTarget) * 100
        : 0
    );
    ...
    categoryData.forEach((catData) => {
      ...
      if (
        catData.savingsPotential >
        topCategories[lowestTopIndex].savingsPotential
      ) {
