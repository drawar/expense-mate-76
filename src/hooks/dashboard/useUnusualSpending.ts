...
/**
 * Representation of a detected spending anomaly
 */
export interface SpendingAnomaly {
  merchantName: string;
  reason: string;
  amount: number;
  currency: Currency; // Track the transaction's original currency
...
/**
 * Hook for detecting unusual spending patterns
 */
...
function detectUnusualSpending(
  ...
  for (const transaction of sortedTransactions) {
    ...
    if (
      transaction.amount >
      stats.avgAmount + stats.stdDevAmount * amountThreshold
    ) {
      const multiplier =
    ...
    if (currentCount >= 3) {
      // At least 3 purchases to be considered unusual frequency
      ...
      if (currentWeeklyFrequency > usualWeeklyFrequency * 2) {
        ...
        const existingFrequencyAnomaly = anomalies.find(
          (a) =>
            a.merchantName === merchantName && a.reason.includes("purchase")
        );
...
function buildMerchantStats(
  ...
  for (const [merchantName, txs] of merchantMap.entries()) {
    ...
    const usualFrequency =
      weeksBetween < 0.5 ? txs.length : txs.length / Math.max(1, weeksBetween);
