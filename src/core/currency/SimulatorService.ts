...
export class SimulatorService {
  ...
  async simulateAllCards(
    ...
    const cardResults: CardCalculationResult[] = results.map((result, index) => {
      if (result.status === 'fulfilled') {
        return result.value;
      } else {
        // Handle individual card failure (Requirement 2.4)
        const paymentMethod = activePaymentMethods[index];
        console.error(`Error calculating rewards for ${paymentMethod.name}:`, result.reason);
  ...
  async simulateSingleCard(
    ...
    try {
      ...
      const calculation = await this.rewardService.calculateRewards(calculationInput);
  ...
  async getMonthlySpending(
    ...
    try {
      ...
      console.error(`Error getting monthly spending for ${paymentMethodId}:`, error);
