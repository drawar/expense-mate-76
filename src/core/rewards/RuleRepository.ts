...
export class RuleRepository {
  ...
  private validateRuleData(
    ...
    if (!ruleData.cardTypeId || ruleData.cardTypeId.trim() === "") {
      logger.error(operation, "Validation failed: cardTypeId is required", {
        ruleData,
      });
  ...
  /**
   * Get all reward rules for a specific card type.
   *
   * This is the preferred method for retrieving rules. Use CardTypeIdService
   * to generate the card type ID from payment method properties.
   *
   * @param cardTypeId - The card type ID (generated by CardTypeIdService)
   * @returns Promise resolving to an array of reward rules for the card type
   * @throws {ValidationError} if cardTypeId is empty
   * @throws {AuthenticationError} if user is not authenticated
   * @throws {PersistenceError} if database operation fails
   *
   * @example
   ...
   *
   * const cardTypeId = cardTypeIdService.generateCardTypeId('Chase', 'Sapphire Reserve');
  ...
  async getRulesForCardType(cardTypeId: string): Promise<RewardRule[]> {
    ...
    logger.info(operation, "Fetching rules for card type", { cardTypeId });

    try {
      // Validate input
      if (!cardTypeId || cardTypeId.trim() === "") {
        logger.error(operation, "Validation failed: cardTypeId is required", {
          cardTypeId,
        });
        throw new ValidationError(
      ...
      logger.debug(operation, "Executing Supabase query", {
        table: "reward_rules",
        filter: { card_type_id: cardTypeId },
      });
      ...
      if (error) {
        ...
        throw new PersistenceError(
          `Failed to fetch rules for card type: ${error.message}`,
          operation,
          { cardTypeId },
          error
        );
      ...
      logger.error(
        operation,
        "Unexpected error fetching rules for card type",
        { cardTypeId },
        error instanceof Error ? error : new Error(String(error))
      );
      throw new PersistenceError(
